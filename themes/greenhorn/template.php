 ...
 *   entirety and paste it here, changing the prefix from theme_ to greenhorn_.
 *   For example:
 *
 *     original: theme_breadcrumb()
 *     theme override: greenhorn_breadcrumb()
 *
 *   where greenhorn is the name of your sub-theme. For example, the
...
if (theme_get_setting('greenhorn_fixed')) {
 ...
 */
 function greenhorn_theme(&$existing, $type, $theme, $path) {
  $hooks = zen_theme($existing, $type, $theme, $path);
  // Add your theme hooks like this:
  
  //drupal_add_js(path_to_theme() .'/jquery-1.4.2.min.js');
  
  $hooks['company_post_node_form'] = array(
        'arguments' => array('form' => NULL),
        'template' => 'company-post-form',
		);
  $hooks['job_post_node_form'] = array(
        'arguments' => array('form' => NULL),
...
function greenhorn_preprocess(&$vars, $hook) {
 ...
 */
/* -- Delete this line if you want to use this function
function greenhorn_preprocess_node(&$vars, $hook) {
  $vars['sample_variable'] = t('Lorem ipsum.');
}
// */
 ...
 */
/* -- Delete this line if you want to use this function
function greenhorn_preprocess_comment(&$vars, $hook) {
  $vars['sample_variable'] = t('Lorem ipsum.');
}
// */
 ...
 */
/* -- Delete this line if you want to use this function
function greenhorn_preprocess_block(&$vars, $hook) {
  $vars['sample_variable'] = t('Lorem ipsum.');
}
// */

/* HoG template.php methods */
/**
 * Override or insert variables into all templates.
 *
 * @param $vars
 ...
 */
///* -- Delete this line if you want to use this function
function greenhorn_preprocess_page(&$vars, $hook) {
	if( arg(0) && arg(0) == 'jobboard'){
		$vars['jobboard'] = TRUE;
	}
	...
	/**
	 * If this is a Job Post, add the name of the company to the browser title.
	 */
	$node = $vars['node'];
    // Check the node  type to make sure we have the one we want
    if ( $node->type == 'job_post' ) {
  	  $title = $vars['head_title'];
	  
	  // get the company node
	  $company = node_load($node->field_company[0]['nid']);
	  $company_name = $company->title;
	  
	  $new = "| " . $company_name . " ";
      $pos = "|";
      $title = str_replace($pos, $new.$pos ,$title);
	...
	$quick_find_buttons = '<div><a href="/resources" title="Search Resources"><img src="/sites/all/themes/horns_of_green/images/common/common_bigBtn_resources.png" alt="Search Resources" /></a>Resources</div>' .
            ...
            /*'<div><a href="/guides" title="View Guides"><img src="/sites/all/themes/horns_of_green/images/common/common_bigBtn_how.png" alt="View Guides" /></a>Guides</div>' .*/
...
*/
function greenhorn_preprocess_search_block_form(&$vars, $hook) {
 ...
 */
function greenhorn_nice_menu_build($menu) {
  $output = '';
...
//function greenhorn_preprocess_views_exposed_form_Search_page_1(&$vars, $hook)
function greenhorn_preprocess_views_exposed_form(&$vars, $hook)
{
